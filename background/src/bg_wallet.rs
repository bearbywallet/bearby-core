...
mod tests_background_wallet {
    ...
    #[tokio::test]
    async fn test_add_ledger_btc_wallet_scans_chains_nonsequential_indexes() {
        use config::bip39::EN_WORDS;
        use pqbip39::mnemonic::Mnemonic;
        use proto::btc_utils::BtcAccountXpubsInput;
        use rpc::network_config::ChainConfig;
        use wallet::bitcoin_wallet::BitcoinWallet;
        use wallet::wallet_init::WalletInit;
        use wallet::wallet_storage::StorageOperations;
        ...
        let offline_btc_conf = ChainConfig {
            ...
            testnet: Some(true),
            ftokens: vec![],
            ..test_data::gen_btc_regtest_conf()
        };
        ...
        let mut btc_xpubs = Vec::new();
        let network = offline_btc_conf.bitcoin_network().unwrap_or(bitcoin::Network::Bitcoin);
        ...
        for &li in &ledger_indexes {
            ...
            btc_xpubs.push(xpubs);
        }

        let wallet_settings: WalletSettings = Default::default();
        let device_salt = session::device::get_device_signature();
        let argon_seed = argon2::derive_key(
            &device_salt,
            b"",
        ...
        let wallet_config = WalletConfig {
            keychain,
            storage: Arc::clone(&bg.storage),
            settings: wallet_settings,
        };

        let wallet = wallet::Wallet::from_ledger(
            wallet::LedgerParams {
                accounts,
                ledger_id: vec![1],
                proof,
        ...
        for (pos, &li) in ledger_indexes.iter().enumerate() {
            ...
            assert_eq!(acc.account_type.value(), li as usize);
            ...
            let chains = wallet
                ...
                .unwrap();
            assert_eq!(chains.len(), 4, "pos={} should have 4 address types (offline fallback)", pos);
